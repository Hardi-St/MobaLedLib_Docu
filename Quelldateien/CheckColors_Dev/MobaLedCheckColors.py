...
PALETTE = {"Room_Col0": "#FF6C00","Room_Col1":"#FFA24F","Room_Col2":"#FFCDA6","Room_Col3":"#FF0000", "Room_Col4":"#00FF00", "Room_Col5":"#0000FF", "Gas Light1":"#323232",
...
#        label.pack(pady=10,padx=10)
        ...
        ttk.Button(button_frame, text=_("Serial Monitor"), command=controller.switch_to_serial_mon).grid(row=0, column=2, padx=10, pady=(10, 4), sticky='n')
        #ttk.Button(button_frame, text=_("Hilfe"), command=lambda: webbrowser.open_new_tab(HELPPAGE_FILENAME)).grid(row=0, column=3, padx=10, pady=(10, 4), sticky='n')
        ttk.Button(button_frame, text=_("Hilfe"), command=controller.call_helppage).grid(row=0, column=3, padx=10, pady=(10, 4), sticky='n')
        ...
        self.old_color_prev = tk.Label(preview_frame, background=old_color[:7],
        ...
        for i, key in enumerate(sdConfig.palette):
            keycolor = sdConfig.palette[key]
            ...
            l = tk.Label(f, background=hexcolor, width=12, height=2,text=text,fg=fontcolor)
...
#        ttk.Label(arduino_frame, text=_('ARDUINO Steuerung')).grid(row=0, column=0, columnspan=3, sticky='', padx=4, pady=4)
    ...
    def _update_preview(self):
        ...
        blue = self.blue.get()
        args = (red,green,blue)
        rc,gc,bc = self._correct_rgb_disp(*args)
        argsc = (rc,gc,bc)
        color = rgb_to_hexa(*argsc)
    ...
    def _palette_save_cmd(self, event):
        ...
        cr,cg,cb = self._correct_rgb_disp(*args)
        cor_args = (cr,cg,cb)
        h,s,v = rgb_to_hsv(*cor_args)
        color_cor = rgb_to_hexa(*cor_args)
        r2,g2,b2 = hsv_to_rgb(h,s,100)
        args2 = (r2,g2,b2)
        color2 = rgb_to_hexa(*args2)
        fontcolor = "#000000"
        event.widget["foreground"] = fontcolor
        event.widget["background"] = color2
        if PERCENT_BRIGHTNESS:                                               # 03.12.19:
            brightness = str(v) + " %"
        else:
            brightness = ""
        ...
        old_r,old_g,old_b = hexa_to_rgb(old_color)
        old_args = (old_r, old_g, old_b)
        ocr,ocg,ocb = self._correct_rgb_disp(*old_args)
        cor_old_color = (ocr,ocg,ocb)
        cor_old_color_hex = rgb_to_hexa(*cor_old_color)
        self.old_color_prev.configure(background=cor_old_color_hex)
